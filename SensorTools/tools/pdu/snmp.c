...
byte* createOIDStr(const char *oid){
  char *copy = (char*)malloc(strlen(oid)+1);
  strcpy(copy, oid);
  size_t oidStrLen = 1;
  byte *oidStr = (byte*)malloc(oidStrLen);
  oidStr[0] = 0x2b; // first two numbers of the oid are fixed translation
  char *tok = strtok(copy, ".");
  tok = strtok(NULL, ".");
  while((tok = strtok(NULL, ".")) != NULL){
    int d = atoi(tok);
    byte *l = (byte*)malloc(0);
    size_t r = encodeOID(d, &l);
    oidStrLen += r;
    oidStr = (byte*)realloc(oidStr, oidStrLen);
    memcpy(&oidStr[oidStrLen-r], l, r);
    free(l);
  }
  byte *l = (byte*)malloc(0);
  size_t r = encodeLen(oidStrLen, &l);
  oidStr = (byte*)realloc(oidStr, oidStrLen+r+1);
...
byte* createGetRequestVBList(const char **oids, size_t num){
  size_t vbListSize = 0;
  byte *vbList = (byte*)malloc(vbListSize);
  for(size_t i = 0; i < num; i++){
    ...
    vbList = (byte*)realloc(vbList, newSize);
  ...
  vbList = (byte*)realloc(vbList, vbListSize+1+r);
...
byte* createPDU(byte type, byte *vblist){
  ...
  byte *pduMsg = (byte*)malloc(pduLen+1+r);
  pduMsg[0] = type;
  memcpy(&pduMsg[1], l, r);
...
byte* createGetRequestMessage(byte version, byte* community, size_t cLen, const char **oids, size_t num){
  ...
  byte *comStr = (byte*)malloc(cLen+r+1);
  ...
  byte *l2 = (byte*)malloc(0);
  size_t r2 = encodeLen(msgLen, &l2);
  byte *msg = (byte*)malloc(msgLen+1+r2);
  msg[0] = SNMP_Sequence;
  memcpy(&msg[1], l2, r2);
...
size_t encodeLen(size_t len, byte **encodedStr){
  if(len <= 0x7f){
    *encodedStr = (byte*)realloc(*encodedStr, 1);
  ...
  *encodedStr = (byte*)realloc(*encodedStr, bytes+1);
...
size_t encodeOID(int oid, byte **encodedStr){
  ...
  *encodedStr = (byte*)realloc(*encodedStr, bytes);
  for(int b = 1; b <= bytes; b++){
